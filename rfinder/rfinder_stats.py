...
class rfi_stats:
    ...
    def make_psf(self, cfg_par):
        ...
        command4 = (
            """ -data-column DATA -gain 0.1 -mgain 1.0 -multiscale-scale-bias 0.6 -fit-beam -no-fit-beam """
            + cfg_par["general"]["rfidir"]
        )
    ...
    def time_chunk(self, cfg_par):
        ...
        endtime = self.time[-1]
        time_chunk = float(cfg_par["rfi"]["chunks"]["time_step"]) * 60.0

        times = np.arange(starttime, endtime + time_chunk * 1.0, time_chunk)

        cfg_par["rfi"]["times"] = times
        startdate = Time(starttime / 3600.0 / 24.0, format="mjd", scale="utc")
        cfg_par["rfi"]["startdate"] = startdate
        startdate.format = "iso"
        startdate.subformat = "date_hm"

        enddate = Time(endtime / 3600.0 / 24.0, format="mjd", scale="utc")
    ...
    def baseline_stats(self, cfg_par):
        ...
        self.logger.info(
            "\t\t Number of baselines < {0:d} m = {1:d}".format(cfg_par["rfi"]["baseline_cut"], int(num_short))
        )
        self.logger.info(
            "\t\t Number of baselines > {0:d} m = {1:d}\n".format(cfg_par["rfi"]["baseline_cut"], int(num_long))
        )
    ...
    def predict_noise(self, cfg_par, channelWidths, interval, flag):
        ...
        self.logger.info("\t Properties of observation")
        nrBaseline = cfg_par["rfi"]["number_baseline"]
        self.logger.info("\t\t Total number of baselines = " + str(nrBaseline))
        self.logger.info("\t\t Total number of channels = " + str(channelWidths.shape[1]))
        cfg_par["rfi"]["total_channels"] = channelWidths.shape[1]
        cfg_par["rfi"]["exptime"] = interval.sum() / nrBaseline / 3600.0
        self.logger.info(
            "\t\t Observing time on source = {0:.5f} h ({1:d} polarisations)\n".format(
                cfg_par["rfi"]["exptime"], polnum
            )
        )
    ...
    def alt_az(self, cfg_par, time):
        ...
        time = Time(time / 3600.0 / 24.0, format="mjd", scale="utc")
