...
class UzeroFlagger:
    ...
    def saveFlags(self,inVis,flagname):

        logger.info('\t Saving flags: '+flagname)

        flg(vis=inVis, mode='save', versionname=flagname)
    ...
    def deleteFlags(self,inVis,flagname):

        logger.info('\t Deleting flags: '+flagname)


        flg(vis=inVis, mode='delete', versionname=flagname)
    ...
    def restoreFlags(self,inVis,flagname):


        logger.info('\t Restoring flags: '+flagname)


        flg(vis=inVis, mode='restore', versionname=flagname)
    ...
    def makeCube(self,inVis,outCubePrefix,kind='scan'):
        ...
        if taper is not None:
            os.system("wsclean -name {outCubePrefix} -j 64 -mem 100 -no-update-model-required -weight briggs {robust} -taper-gaussian {taper} -size {imsize} {imsize} -scale {cell}asec -channels-out 1 -pol I -channel-range {chanmin} {chanmax} -niter 0 -auto-threshold 0.5 -auto-mask 10.0 -gain 0.2 -mgain 0.85 -multiscale-scale-bias 0.6 -padding 1.2 -quiet {inVis}".format(outCubePrefix=outCubePrefix,robust=robust,taper=taper,imsize=imsize,cell=cell,chanmin=chMin,chanmax=chMax,inVis=inVis))

        else:    #cell =2.asec imsize=3600
            os.system("wsclean -name {outCubePrefix} -j 64 -mem 100 -no-update-model-required -weight briggs {robust} -size {imsize} {imsize} -scale {cell}asec -channels-out 1 -pol I -channel-range {chanmin} {chanmax} -niter 0 -auto-threshold 0.5 -auto-mask 10.0 -gain 0.2 -mgain 0.85 -multiscale-scale-bias 0.6 -padding 1.2 -quiet {inVis}".format(outCubePrefix=outCubePrefix,robust=robust,taper=taper,imsize=imsize,cell=cell,chanmin=chanMin,chanmax=chanMax,inVis=inVis))
    ...
    def saveFFTTable(self,inFFT,inFFTHeader,visName, U, V, galaxy, msid, track, scan, el, az, method, threshold, dilateU, dilateV, makePlots):
        ...
        if cutoff>np.nanmax(tabArr['Amp']):
            willflag = False
            logger.warn("Cutoff is larger than max amplitude. Notihng will be flagged.")
    ...
    def sunBlockStats(self, inFFTData, galaxy, msid, track, scan, makePlots, threshmode='mad', threshold=300., ax=None, title='', verb=True):
        ...
        if verb:
            logger.info('\tFFT grid has {:d} nonzero points.'.format(npoints))
        if npoints < 3:
            logger.info('\tThis is not sufficient for any statistics, returning no flags.')
     ...
     #   if threshmode == 'fit' or ax != None:
        ...
        if makePlots == True :
            ...
            logger.warn('For some reasons I am not making the fftstats plots!')
    ...
    def putFlags(self,pf_inVis, pf_inVisName, pf_stripeFlags):
        ...
        self.saveFlags(pf_inVisName,flagname='stripe_flag_after')

        return 0


    def run_flagUzeros(self,msname):
        ...
        datapath=self.config['general']['outdir']
        mfsOb = msname

        self.setDirs(datapath)
        ...
        logger.info("====================================================")
        logger.info('Starting the flag_u_zeros segment')
        ...
        for ii in range (0,len(obsIDs)):
            ...
            if os.path.exists(inVis+'.flagversions'):
                ...
                if 'stripe_flag_before' in fvers:
                    ...
                    self.restoreFlags(inVisName,flagname='stripe_flag_before')
                    while fvers[-1] != 'stripe_flag_before':
                        self.deleteFlags(inVisName,flagname=fvers[-1])
                        fvers = fvers[:-1]
                else:
                    logger.info("Before we start, save flag version 'stripe_flag_before'")
                    self.saveFlags(inVisName,flagname='stripe_flag_before')
            else:
                logger.info("Before we start, save flag version 'stripe_flag_before'")
                self.saveFlags(inVisName,flagname='stripe_flag_before')
            ...
            logger.info("----------------------------------------------------")
            logger.info("Imaging full MS for stripe analysis")
            ...
            self.makeCube(inVisName,outCubePrefix)
            ...
            for kk in range(len(scanNums)):
                ...
                self.saveFlags(visName,flagname='scan_flags_start')
                ...
                self.makeCube(visName,outCubePrefix_0)
                ...
                for threshold in thresholds:
                    ...
                    self.restoreFlags(visName,flagname='scan_flags_start')

                    while fvers[-1] != 'scan_flags_start':
                        self.deleteFlags(visName,flagname=fvers[-1])
                        fvers = fvers[:-1]
                    ...
                    if os.path.exists(outCubeName):
                        os.remove(outCubeName)
                    self.makeCube(visName,outCubePrefix)
                    fitsdata = fits.open(outCubeName)
