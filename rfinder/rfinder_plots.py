...
class rfi_plots:
    ...
    def plot_rfi_imshow(self, cfg_par, time_step=-1):
        ...
        if os.path.exists(rfi_freq_base) == False:
            ...
            data = t[0].data
            prihdr = t[0].header
            freqs = (np.linspace(1, data.shape[1], data.shape[1]) - prihdr["CRPIX1"]) * prihdr["CDELT1"] + prihdr[
                "CRVAL1"
            ]
            ...
            if bandwidth <= 25:
                ...
                step_bin = 10
            elif bandwidth <= 150.0 and bandwidth > 50.0:
                step_bin = 25.0
            elif bandwidth <= 500.0 and bandwidth > 150.0:
                step_bin = 50.0
            else:
                step_bin = 100.0
            # Smaller multiple
            ...
            if cfg_par["rfi"]["chunks"]["time_enable"] == False:
                ...
                time_del = TimeDelta(time_delta * 60.0, format="sec")
                time_delta_plus = TimeDelta(float(cfg_par["rfi"]["chunks"]["time_step"]) * 60.0, format="sec")
            ...
            if cfg_par["rfi"]["RFInder_mode"] == "rms_clip":
                ...
                title_plot = "Stokes:  {0:s} / {1:s} / {2:%d}{2:%b}{2:%y}: {2:%H}:{2:%M} - {3:%H}:{3:%M}".format(
                    pol, rfi_clip, start.datetime, end.datetime
                )
            if cfg_par["rfi"]["RFInder_mode"] == "use_flags":
                title_plot = "Stokes:  {0:s} / {1:s} / {2:%d}{2:%b}{2:%y}: {2:%H}:{2:%M} - {3:%H}:{3:%M}".format(
                    pol, "Flags", start.datetime, end.datetime
                )
    ...
    def plot_noise_frequency(self, cfg_par, time_step=-1):
        ...
        if os.path.exists(rfi_table) == False:
            ...
            ax1.set_xlim(
                [
                    (cfg_par["rfi"]["lowfreq"] - 5.0 * cfg_par["rfi"]["chan_widths"]) / 1e6,
                    (cfg_par["rfi"]["highfreq"] + 5.0 * cfg_par["rfi"]["chan_widths"]) / 1e6,
                ]
            )
            ...
            if cfg_par["plots"]["plot_details"]["plot_noise"] != "rfi":
                ...
                if np.isnan(np.sum(noise_all)):
                    noise_all = np.zeros([len(freqs)]) + 100.0
                if np.isnan(np.sum(noise_short)):
                    noise_short = np.zeros([len(freqs)]) + 100.0
                if np.isnan(np.sum(noise_long)):
                    noise_long = np.zeros([len(freqs)]) + 100.0
    ...
    def plot_altaz(self, cfg_par, number_chunks):
        ...
        for j in range(0, len(freqs_bin)):
            ...
            azimuth, altitude = self.aitoff(azimuth - 180.0, altitude)
            # azimuth=np.array([azimuth],dtype=float)
            # altitude=np.array([altitude],dtype=float)
            ...
            asse = ax_centre.scatter(
                ...
                linewidth=0.5,
                edgecolors="black",
                label=r"Stokes: {0:s}".format(pol),
            )

            azs = np.arange(-180.0, 181.0, 45.0)
            als = np.arange(0.0, 91.0, 1.0)
            azs[azs == 0] = 1e-6
            als[als == 0] = 1e-6
            for counter in azs:
            ...
            azs = np.arange(-180.0, 181.0, 1.0)
            als = np.arange(0.0, 91.0, 15.0)
            azs[azs == 0] = 1e-6
            als[als == 0] = 1e-6
            for al in als:
            ...
            for dd in [15.0, 30.0, 45.0, 60.0, 75.0]:
                [X, Y] = self.aitoff(-180.0, dd)
                ax_centre.text(
                    X, Y, r"${0:s}$".format(str(int(round(dd, 0)))), horizontalalignment="right", fontsize="medium"
                )
            ...
            if cfg_par["rfi"]["RFInder_mode"] == "rms_clip":
                rfi_clip = str(cfg_par["rfi"]["rms_clip"]) + r"$\sigma$ clip"
                title_plot = "{0:d}-{1:d} MHz / {2:s} / {3:%d}{3:%b}{3:%y}: {3:%H}:{3:%M} - {4:%H}:{4:%M}".format(
                    start_freq, end_freq, rfi_clip, start.datetime, end.datetime
                )
            if cfg_par["rfi"]["RFInder_mode"] == "use_flags":
                title_plot = "{0:d}-{1:d} MHz / {2:s} / {3:%d}{3:%b}{3:%y}: {3:%H}:{3:%M} - {4:%H}:{4:%M}".format(
                    start_freq, end_freq, "Flags", start.datetime, end.datetime
                )
            ...
            ax_x.scatter(
                azimuth, flags, c=flags, cmap=colormap, vmin=0, vmax=100.0, s=70, linewidth=0.5, edgecolors="black"
            )
            ...
            ax_y.scatter(
                flags, altitude, c=flags, cmap=colormap, vmin=0, vmax=100.0, s=70, linewidth=0.5, edgecolors="black"
            )
    ...
    def plot_altaz_short(self, cfg_par):
        ...
        asse = ax_centre.scatter(
            ...
            linewidth=0.5,
            edgecolors="black",
            label=r"${0:s}$".format(cfg_par["general"]["fieldname"]),
        )
        ...
        title_plot = "{0:d}-{1:d} MHz / {2:%d}{2:%b}{2:%y}: {2:%H}:{2:%M} - {3:%H}:{3:%M}".format(
            start_freq, end_freq, start.datetime, end.datetime
        )
    ...
    def plot_summary_stats(self, flag_stats, cfg_par, key="corr"):
        ...
        if key in ["antenna", "ant"]:
            ...
            ax2.bar(
                list(range(len(flag_stats.keys()))), flag_stats.values(), color="orange", ec="orange", align="center"
            )
            plt.savefig(summaryplot)
            if cfg_par["plots"]["plot_details"]["plot_eps"]:
                plt.savefig(summaryplot.replace("png", "eps"), format="eps")
