...
class Rfinder:
    ...
    def go(self, cfg_par):
        ...
        if self.cfg_par[task]["rfi_enable"] == True:
            if self.cfg_par[task]["chunks"]["time_enable"] == True:
                ...
                for i in range(len(times) - 1):
                    ...
                    time_delta_plus = TimeDelta(float(self.cfg_par["rfi"]["chunks"]["time_step"]) * 60.0, format="sec")
                    start = self.cfg_par["rfi"]["startdate"] + time_del
                    end = start + time_delta_plus
                    self.logger.info((" ------ Working on chunk #{0:d}:").format(i))
                    self.logger.info(
                        ("\tbetween {0:%d}{0:%b}{0:%y}: {0:%H}:{0:%M} - {1:%H}:{1:%M}\n").format(
        ...
        if self.cfg_par[task]["plot_details"]["enable"] == True:
            if self.cfg_par["rfi"]["chunks"]["time_enable"] == True:
                ...
                for i in range(len(times) - 1):
                    ...
                    self.logger.info(
                        ("\t \t between {0:%d}{0:%b}{0:%y}: {0:%H}:{0:%M} - {1:%H}:{1:%M}").format(
                            start.datetime, end.datetime
                        )
                    )
                ...
                if (
                    self.cfg_par["plots"]["plot_details"]["movies"]["altaz_gif"] == True
                    or self.cfg_par["plots"]["plot_details"]["movies"]["2d_gif"] == True
                    or self.cfg_par["plots"]["plot_details"]["movies"]["1d_gif"] == True
                ):
        ...
        if self.cfg_par[task]["plot_summary"]["enable"] == True:
            ...
            if summary_results:
                self.logger.info(
                    f"------ Total % Flagged: {round(sum(summary_results[axis].values()) / len(summary_results[axis].values()), 2)} ------"
                )
                json_file = cfg_par["general"]["rfidir"] + "summary.json"
