...
def rfi_frequency(cfg_par, time_step=-1):
    ...
    if os.path.exists(rfi_freq_base) == False:
        ...
        freqs = (np.linspace(1, datacube.shape[1], datacube.shape[1]) - prihdr["CRPIX1"]) * prihdr["CDELT1"] + prihdr[
            "CRVAL1"
        ]
        ...
        flag_lin_long = np.zeros([datacube.shape[1]]) + (100.0 - 100.0 / more_long)
        flag_lin_short = np.zeros([datacube.shape[1]]) + (100.0 - 100.0 / more_short)
        ...
        for i in range(0, datacube.shape[1]):
            ...
            lenghts = np.array([cfg_par["rfi"]["baseline_lenghts"]]) + 0.0
            idx = (np.abs(lenghts - baseline_cutoff)).argmin()

            shortbase = datacube[:idx, i]
            longbase = datacube[idx:, i]

            rms_lin_tmp = 1.0 - np.divide(np.divide(np.nansum(datacube[:, i]), datacube.shape[0]), 100.0)
            rms_lin[i] = np.divide(1.0, np.sqrt(rms_lin_tmp))

            flag_lin_tmp = np.divide(np.nansum(shortbase), len(shortbase))
            flag_lin_short[i] = flag_lin_tmp
            rms_lin_tmp_short = 1.0 - np.divide(np.divide(np.nansum(shortbase), len(shortbase)), 100.0)
            rms_lin_short[i] *= np.divide(1.0, np.sqrt(rms_lin_tmp_short))

            flag_lin_tmp = np.divide(np.nansum(longbase), len(longbase))
            flag_lin_long[i] = flag_lin_tmp
            rms_lin_tmp_long = 1.0 - np.divide(np.divide(np.nansum(longbase), len(longbase)), 100.0)
            rms_lin_long[i] *= np.divide(1.0, np.sqrt(rms_lin_tmp_long))
...
def write_html_fullreport(cfg_par):
    ...
    env = Environment(loader=FileSystemLoader(cfg_par["general"]["template_folder"]))
        ...
        with open(cfg_par["general"]["rfidir"] + "time_report.html", "w") as f:
            ...
            f.write(
                template.render(
                    ...
                    minbase=str(np.round(lenghts[0][0], 0)),
                    totbase=cfg_par["rfi"]["number_baseline"],
                    exptime=np.round(cfg_par["rfi"]["exptime"] * 60.0, 2),
                    polnum=cfg_par["rfi"]["polnum"],
                    noise=np.round(cfg_par["rfi"]["theo_rms"][0] * 1e3, 5),
