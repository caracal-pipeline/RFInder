...
class rfi:
    ...
    def load_from_ms(self, cfg_par, times=0, counter=0):
        ...
        self.coords = self.coords * 180.0 / np.pi
        cfg_par["rfi"]["coords"] = SkyCoord(
            self.coords[self.selectFieldID, :, 0] * u.degree,
            self.coords[self.selectFieldID, :, 1] * u.degree,
            unit=(u.deg, u.deg),
        )
        ...
        if counter != 0:
            ...
            if np.sum(selection) != 0.0:
                ...
                self.logger.warning(
                    "\t     Check the observation's scan/field timing - this time window may not include the selected field ###"
                )
                empty_table = 1
            t2.close()
        ...
        for i in range(0, self.nant - 1):
            for j in range(i + 1, self.nant):
                ...
                if self.aperfi_badant != None:
                    if (
                        self.ant_names[i] != self.ant_names[j]
                        and any(x != self.ant_names[i] for x in self.aperfi_badant)
                        and any(x != self.ant_names[i] for x in self.aperfi_badant)
                    ):
    ...
    def priors_flag(self, cfg_par):
        ...
        self.datacube = np.zeros(
            [
                len(self.baselines_sort),
                self.flag.shape[1],
                self.flag.shape[0] // len(self.baselines_sort),  # Use // for integer division
            ]
        )
        ...
        for i in range(self.flag.shape[0]):
            if self.aperfi_badant not in [None, []]:
                if any(x == self.ant1[i] for x in self.aperfi_badant) or any(
                    x == self.ant2[i] for x in self.aperfi_badant
                ):
                    self.flag[i, :, 0] = True
            if self.ant1[i] == self.ant2[i]:
                ...
                if cfg_par["rfi"]["RFInder_mode"] == "use_flags":
                    ...
                    if pol == "yx" or pol == "YX":
                        ...
                        self.datacube[indice, :, counter] = np.abs(self.vis[i, :, 0]) - np.abs(self.vis[i, :, 1]) / 2.0
    ...
    def find_rfi(self, datas, cfg_par, time_step=-1):
        ...
        if self.aperfi_rfifree_min < float(cfg_par["rfi"]["lowfreq"]) or self.aperfi_rfifree_min > float(
        ...
        if self.aperfi_rfifree_max < float(cfg_par["rfi"]["lowfreq"]) or self.aperfi_rfifree_max > float(
            cfg_par["rfi"]["highfreq"]
        ):
            ...
            for i in range(0, self.datacube.shape[0]):
                if self.datacube.shape[2] == 0:
                    ...
                    for j in range(0, self.datacube.shape[1]):
                        ...
                        if tmp_over == 1.0:
                            tmp_over = 0.0
                        rms[i, j] = 100.0 * tmp_over / time_ax_len

        elif cfg_par["rfi"]["RFInder_mode"] == "use_flags":
            for i in range(0, self.datacube.shape[0]):
                for j in range(0, self.datacube.shape[1]):
                    rms[i, j] = 100.0 * np.nansum(self.datacube[i, j, :]) / time_ax_len
